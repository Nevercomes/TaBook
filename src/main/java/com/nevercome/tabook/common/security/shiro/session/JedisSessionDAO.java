...
public class JedisSessionDAO extends AbstractSessionDAO implements SessionDAO {
    ...
    public void delete(Session session) {
        ...
        try {
            ...
            logger.error("delete {} ", session.getId());
        } finally {
            JedisUtils.returnResource(jedis);
        }
    }
            ...
            for (Map.Entry<String, String> e : map.entrySet()) {
                if (StringUtils.isNotBlank(e.getKey()) && StringUtils.isNotBlank(e.getValue())) {
                    ...
                    if (ss != null && ss.length == 3) {
                        ...
                        } catch (Exception e2) { // Session 校验失败
                            jedis.hdel(sessionKeyPrefix, e.getKey());
                        }
                    } else { // 存储的Session不规范
                        jedis.hdel(sessionKeyPrefix, e.getKey());
                    }
                } else if (StringUtils.isNotBlank(e.getKey())) { // 存储的Session无Value
                    jedis.hdel(sessionKeyPrefix, e.getKey());
                }
            }
    ...
    protected Session doReadSession(Serializable sessionId) {
        ...
        // sun 从request中获取session 不频繁访问缓存
        if (request != null) {
            String uri = request.getServletPath();
            if (Servlets.isStaticFile(uri)) {
                return null;
